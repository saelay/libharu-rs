...
use libharu::{Point, Document, Page, LineCap, LineJoin};

fn draw_line<T: Into<Point>>(page: &Page, pos: T, label: &str) -> anyhow::Result<()> {
    let pos = pos.into();
...
fn draw_line2<T: Into<Point>>(page: &Page, pos: T, label: &str) -> anyhow::Result<()> {
    let pos = pos.into();
...
fn draw_rect<T: Into<Point>>(page: &Page, pos: T, label: &str) -> anyhow::Result<()> {
    let pos = pos.into();
    page.begin_text()?;
    page.move_text_pos((pos.x, pos.y -10.0))?;
    page.show_text(label)?;
